...
def convert(hf_model, tensor_parallel_size, new_state_dicts, use_te, hidden_size=1024, num_heads=16, kv_channels=64):
    ...
    for j in range(num_heads):
        for i in range(kv_channels):
            order[i + kv_channels*3*j] = j*kv_channels+i
            order[kv_channels + i + kv_channels*3*j] = j*kv_channels+i+num_heads*kv_channels
            order[kv_channels*2 + i + kv_channels*3*j] = j*kv_channels+i+num_heads*kv_channels*2
    ...
    for name, tensor in hf_state_dict.items():
        ...
        if "embeddings.class_embedding" in name:
            ...
            if tensor_parallel_size == 1:
                ...
                num_padded_heads = ((num_heads + tensor_parallel_size - 1) // tensor_parallel_size) * tensor_parallel_size #32
            else:
        ...
        if use_te and is_extra_state_layer:
            ...
            if layer in extra_state_layers:
                ...
                for i in range(tensor_parallel_size):
                    new_state_dicts[i]["model"][prefix + extra_state_name] = None
...
if __name__ == "__main__":
    ...
    convert(args.model_name, args.tensor_parallel_size, args.use_te)
